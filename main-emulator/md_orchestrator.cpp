...
CMDOrchestrator::CMDOrchestrator(FATFS *pFileSystem)
...
CMDOrchestrator::~CMDOrchestrator() {
    if (m_pRomBuffer) {
        delete[] m_pRomBuffer;
    }
    for (int i = 0; i < 6; i++) {
...
boolean CMDOrchestrator::Initialize() {
...
boolean CMDOrchestrator::SaveState(int slot) {
...
void CMDOrchestrator::LoadState(int slot) {
    if (!m_bRomLoaded) return;
...
boolean CMDOrchestrator::IsPAL() const {
    return Pico.m.pal ? TRUE : FALSE;
}

void CMDOrchestrator::CaptureRewindState() {
